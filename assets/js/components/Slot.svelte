<script lang="ts">
  ...
  export let selected;
  export let on_slot_click;
  export let volume_booked;

  function handle_click() {
    on_slot_click(booking);
  }
</script>
...
<button
  ...
  ${available && !selected ? 'hover:border-gray-400' : ''}
  ${available && selected ? 'hover:bg-brand-surface-hover' : ''}
...
>
  <p class="text-center relative text-lg font-semibold">
    <slot />
  </p>

  <small class="text-xs flex items-center gap-1">
    {#if volume_booked > 0}
      {volume_booked}
      <svg
        width="15"
